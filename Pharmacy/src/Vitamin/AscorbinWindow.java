...
public class AscorbinWindow extends JFrame {
    private String userEmail;
    private ArrayList<String> selectedMedicines;
    private int count = 0;
    private JLabel countLabel;

    public AscorbinWindow(String userEmail, ArrayList<String> selectedMedicines) {
        ...
        double price = MedicineDatabase.getPrice("Аскорбинка");

        JPanel panel = new JPanel(new BorderLayout());
        panel.setBackground(new Color(200, 230, 229));
        panel.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));

        ImageIcon icon = new ImageIcon("exam/src/main/resources/Images/Vit/askorbin.jpg");
        ...
        JLabel nameLabel = new JLabel("Аскорбинка - " + price + " тг.");
        nameLabel.setFont(new Font("Arial", Font.BOLD, 18));

        JLabel descLabel = new JLabel("<html>Витамин C, способствует укреплению иммунитета и обладает антиоксидантным эффектом.</html>");
        ...
        backButton.addActionListener(e -> {
            dispose();
            new VitaminsWindow(userEmail, selectedMedicines);
        });
    ...
    private void addToBasket() {
        if (count > 0) {
            for (int i = 0; i < count; i++) {
                selectedMedicines.add("Аскорбинка");
            }
        }
    }
