...
public class Espumizan extends JFrame {
    private String userEmail;
    private ArrayList<String> selectedMedicines;
    private int count = 0;
    private JLabel countLabel;

    public Espumizan(String userEmail, ArrayList<String> selectedMedicines) {
        ...
        double price = MedicineDatabase.getPrice("Эспумизан");

        JPanel panel = new JPanel(new BorderLayout());
        panel.setBackground(new Color(200, 230, 229));
        panel.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));

        ImageIcon icon = new ImageIcon("exam/src/main/resources/Images/Moms/espumizan.png");
        ...
        JLabel nameLabel = new JLabel("Эспумизан - " + price + " тг.");
        nameLabel.setFont(new Font("Arial", Font.BOLD, 18));

        JLabel descLabel = new JLabel("<html>Устраняет вздутие живота у младенцев.<br>Быстро облегчает колики и газы.</html>");
    ...
    private void addToBasket() {
        if (count > 0) {
            for (int i = 0; i < count; i++) {
                selectedMedicines.add("Эспумизан");
            }
        }
    }
